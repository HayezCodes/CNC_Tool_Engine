"""Sandvik Coromant structured JSON adapter for the Enterprise Tooling Search system.

Parses Sandvik Coromant-style structured JSON tooling records into normalized tooling
search records matching grade_engine.tooling_search.SCHEMA_FIELDS.

Sandvik Coromant background:
  Sandvik Coromant (part of Sandvik AB) is a Swedish precision tooling manufacturer
  and one of the world's largest cutting tool suppliers. Key product families include:
  CoroTurn (turning inserts and toolholders), CoroMill (milling inserts and cutters),
  CoroDrill (solid carbide and indexable drilling), CoroThread (threading inserts),
  CoroCut (grooving and parting), Silent Tools (vibration-damped boring), and
  CoroMill Plura (solid carbide endmills). Grades use Tiger·tec Silver PVD/CVD coating
  families (GC4000 series).

Adapter-specific notes:
  - Indexable turning: TurningInsert, IndexableTurningInsert → 'turning_insert'
  - Milling: MillingInsert, ShoulderMillingInsert, FaceMillingInsert → 'milling_insert'
  - High-feed: HighFeedMillingInsert, HighFeedInsert → 'high_feed_insert'
  - Solid carbide drills: SolidCarbideDrill → 'drill'
  - Indexable drills: IndexableDrill, DrillInsert → 'indexable_drill'
  - Solid carbide endmills: SolidCarbideEndmill, Endmill → 'endmill'
  - Grooving: GroovingInsert, PartingInsert → 'grooving_insert'
  - Threading: ThreadingInsert → 'threading_insert'
  - Boring bars: BoringBar, SilentToolsBar → 'boring_bar'
  - Taps: SpiralTap, MachineTap → 'tap'
  - Thread mills: ThreadMill → 'thread_mill'
  - Reamers: Reamer, SolidCarbideReamer → 'reamer'

Scope (safe subset only):
  - Identity, technical, source traceability fields
  - Dimensions: always {} — never imported
  - No feeds/speeds at any stage

Out of scope (enforced):
  - Any JSON key containing feed, speed, sfm, rpm, ipr, ipm, vc, or fz causes
    the entire record to be rejected.
...
_TOOL_CATEGORY_MAP: dict[str, str] = {
    ...
    "silenttools": "boring_bar",
    "silenttoolsbar": "boring_bar",
    "spiraltap": "tap",
    "machinetap": "tap",
...
class SandvikCoromantAdapter(BaseToolingAdapter):
    """Parse Sandvik Coromant-style structured JSON tooling records into normalized records.
    ...
    Usage::
        ...
        adapter = SandvikCoromantAdapter()
    ...
    SOURCE_FORMAT = "sandvik_coromant_json"

    def parse(self, source: str | Path) -> list[dict[str, Any]]:
        """Parse a Sandvik Coromant structured JSON file and return normalized records."""
        self._errors = []
    ...
    def parse_json_string(self, json_string: str) -> list[dict[str, Any]]:
        """Parse a Sandvik Coromant JSON string and return normalized tooling records."""
        self._errors = []
        self._rejected_count = 0
...
def parse_sandvik_coromant_file(source: str | Path) -> dict[str, Any]:
    """Parse a Sandvik Coromant structured JSON file and return a result summary dict."""
    adapter = SandvikCoromantAdapter()
