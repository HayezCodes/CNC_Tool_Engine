"""Mitsubishi Materials structured JSON adapter for the Enterprise Tooling Search system.

Parses Mitsubishi-style structured JSON tooling records (as exported or transcribed
from catalog sources) into normalized tooling search records matching
grade_engine.tooling_search.SCHEMA_FIELDS.

Input format: JSON object with a catalog_header and a list of tool_records.
Each tool_record is a flat dict with Mitsubishi-convention field names.

Scope (safe subset only):
  - Identity: brand (always Mitsubishi Materials), tool_category, series,
...
BRAND = "Mitsubishi Materials Corporation"

# ── Tool category mapping ─────────────────────────────────────────────────────
# Maps Mitsubishi JSON tool_type values (normalized) to schema tool_category.
...
class MitsubishiMaterialsAdapter(BaseToolingAdapter):
    """Parse Mitsubishi-style structured JSON tooling records into normalized records.
    ...
    Usage::
        ...
        adapter = MitsubishiMaterialsAdapter()
    ...
    SOURCE_FORMAT = "mitsubishi_materials_json"

    def parse(self, source: str | Path) -> list[dict[str, Any]]:
        """Parse a Mitsubishi structured JSON file and return normalized records."""
        self._errors = []
    ...
    def parse_json_string(self, json_string: str) -> list[dict[str, Any]]:
        """Parse a Mitsubishi JSON string and return normalized tooling records.
    ...
    def _normalize_record(
        ...
        *,
        ...
        Records are rejected if any JSON key contains a forbidden feed/speed term.
        """
        mpn = str(raw.get("part_number", "")).strip()

        # Guard: reject records whose keys contain forbidden feed/speed terms
        for key in raw:
...
def parse_mitsubishi_file(source: str | Path) -> dict[str, Any]:
    """Parse a Mitsubishi structured JSON file and return a result summary dict.
