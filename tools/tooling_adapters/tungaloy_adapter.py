"""Tungaloy structured JSON adapter for the Enterprise Tooling Search system.

Tungaloy Corporation (part of IMC Group) is a Japanese cutting tool manufacturer.
Key product families: TungTurn turning inserts (AH/T grades), TungMeister milling,
DoFeedMill high-feed milling, TungDrill drills, TungEnd solid carbide endmills,
TungCut grooving/parting, TungThread threading, and EZBore boring bars.
...
class TungaloyAdapter(BaseToolingAdapter):
    """Parse Tungaloy-style structured JSON tooling records into normalized records."""

    SOURCE_FORMAT = "tungaloy_json"
...
def parse_tungaloy_file(source: str | Path) -> dict[str, Any]:
    """Parse a Tungaloy structured JSON file and return a result summary dict."""
    adapter = TungaloyAdapter()
