"""Kyocera structured JSON adapter for the Enterprise Tooling Search system.

Kyocera Corporation (Kyocera Precision Tools) is a Japanese cutting tool manufacturer.
Key product families: PR/CA/PV grade turning inserts, MFH Miracle milling, MAS/MDS drills,
solid carbide endmills, GBA/GBR grooving inserts, threading inserts, and boring bars.
...
class KyoceraAdapter(BaseToolingAdapter):
    """Parse Kyocera-style structured JSON tooling records into normalized records."""

    SOURCE_FORMAT = "kyocera_json"
...
def parse_kyocera_file(source: str | Path) -> dict[str, Any]:
    """Parse a Kyocera structured JSON file and return a result summary dict."""
    adapter = KyoceraAdapter()
