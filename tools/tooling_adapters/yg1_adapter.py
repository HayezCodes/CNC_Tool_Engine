"""YG-1 structured JSON adapter for the Enterprise Tooling Search system.

YG-1 Tool Company is a Korean cutting tool manufacturer known for drills, endmills,
taps, thread mills, and reamers. Key product families: Dream Drill, Dream EMS endmills,
Hi-Pro Syn taps, thread mills, solid carbide reamers, and indexable inserts.
...
class YG1Adapter(BaseToolingAdapter):
    """Parse YG-1-style structured JSON tooling records into normalized records."""

    SOURCE_FORMAT = "yg1_json"
...
def parse_yg1_file(source: str | Path) -> dict[str, Any]:
    """Parse a YG-1 structured JSON file and return a result summary dict."""
    adapter = YG1Adapter()
