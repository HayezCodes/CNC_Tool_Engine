"""Kennametal structured JSON adapter for the Enterprise Tooling Search system.

Kennametal Inc. is a US-based cutting tool and tooling systems manufacturer.
Key product families: Beyond coating turning inserts (KC5010/KC5025 grades),
HARVI solid carbide endmills, GO Drill solid carbide drills, KGOP grooving inserts,
KM quick-change toolholding, Mill 4-12 milling inserts, and threading inserts.
...
class KennametalAdapter(BaseToolingAdapter):
    """Parse Kennametal-style structured JSON tooling records into normalized records."""

    SOURCE_FORMAT = "kennametal_json"
...
def parse_kennametal_file(source: str | Path) -> dict[str, Any]:
    """Parse a Kennametal structured JSON file and return a result summary dict."""
    adapter = KennametalAdapter()
