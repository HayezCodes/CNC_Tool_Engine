...
INPUT_PATH = REPO_ROOT / "tool_data" / "catalog_ingestion" / "staged" / "helical_solutions_endmill_families.json"
...
def build_reviewed_helical_records() -> list[dict]:
    return [
...
def review_helical_family_records() -> Path:
    records = build_reviewed_helical_records()
    errors: list[str] = []
...
if __name__ == "__main__":
    output_path = review_helical_family_records()
    print(f"Saved {len(build_reviewed_helical_records())} reviewed Helical Solutions family records to {output_path}.")
