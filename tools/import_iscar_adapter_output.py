"""Import Iscar adapter output into the Enterprise Tooling Search index.
...
Validates using the standard tooling import workflow, then writes to:
    tool_data/tooling_search/records/iscar_imported_tools.json
...
DEFAULT_OUTPUT = RECORDS_DIR / "iscar_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_iscar_sample.py' first.")
        return 1
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import Iscar adapter output into the Enterprise Tooling Search index."
    )
    parser.add_argument(
        "--output",
