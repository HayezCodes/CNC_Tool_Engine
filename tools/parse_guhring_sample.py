"""Runner script: parse the Guhring sample JSON fixture.

Parses tools/tooling_adapters/samples/sample_guhring_structured.json,
...
Usage::
    ...
    python tools/parse_guhring_sample.py --output path/to/custom.json
"""
...
from tools.tooling_adapters.guhring_adapter import parse_guhring_file
...
SAMPLE_JSON = (
    ...
    / "tooling_adapters"
    / "samples"
    / "sample_guhring_structured.json"
)
DEFAULT_OUTPUT = (
    Path(__file__).resolve().parent
    / "tooling_adapters"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    result = parse_guhring_file(SAMPLE_JSON)
    ...
    for rec in result["records"]:
        ...
        print(
            f"  [{rec['tool_category']:20s}] {rec['manufacturer_part_number']:32s} "
            f"  {status} / {cds}"
        )
    ...
    print("  2. Run: python tools/import_guhring_adapter_output.py")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse the Guhring sample JSON fixture and write normalized JSON output."
    )
    parser.add_argument(
        "--output",
