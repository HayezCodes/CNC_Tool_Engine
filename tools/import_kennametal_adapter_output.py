"""Import Kennametal adapter output into the Enterprise Tooling Search index.
...
ADAPTER_OUTPUT = _REPO_ROOT / "tools" / "tooling_adapters" / "output" / "kennametal_sample_records.json"
DEFAULT_OUTPUT = RECORDS_DIR / "kennametal_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_kennametal_sample.py' first.")
        return 1
