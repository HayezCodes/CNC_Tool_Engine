"""Import YG-1 adapter output into the Enterprise Tooling Search index.
...
ADAPTER_OUTPUT = _REPO_ROOT / "tools" / "tooling_adapters" / "output" / "yg1_sample_records.json"
DEFAULT_OUTPUT = RECORDS_DIR / "yg1_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_yg1_sample.py' first.")
        return 1
