"""Import Seco Tools adapter output into the Enterprise Tooling Search index.
...
ADAPTER_OUTPUT = _REPO_ROOT / "tools" / "tooling_adapters" / "output" / "seco_sample_records.json"
DEFAULT_OUTPUT = RECORDS_DIR / "seco_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_seco_sample.py' first.")
        return 1
