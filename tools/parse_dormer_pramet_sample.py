"""Runner script: parse the Dormer Pramet sample JSON fixture.

Parses tools/tooling_adapters/samples/sample_dormer_pramet_structured.json,
...
Usage::
    ...
    python tools/parse_dormer_pramet_sample.py --output path/to/custom.json
"""
...
from tools.tooling_adapters.dormer_pramet_adapter import parse_dormer_pramet_file
...
SAMPLE_JSON = (
    ...
    / "tooling_adapters"
    / "samples"
    / "sample_dormer_pramet_structured.json"
)
DEFAULT_OUTPUT = (
    Path(__file__).resolve().parent
    / "tooling_adapters"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    result = parse_dormer_pramet_file(SAMPLE_JSON)
    ...
    print()
    print("Next steps:")
    print("  1. Run: python tools/import_dormer_pramet_adapter_output.py")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse the Dormer Pramet sample JSON fixture and write normalized JSON output."
    )
    parser.add_argument(
        "--output",
