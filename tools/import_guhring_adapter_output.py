"""Import Guhring adapter output into the Enterprise Tooling Search index.

Reads:
    tools/tooling_adapters/output/guhring_sample_records.json

Validates using the standard tooling import workflow, then writes to:
    tool_data/tooling_search/records/guhring_imported_tools.json

This script is the second stage of the adapter pipeline, after the adapter runner
has produced its output.  Forbidden feed/speed fields cause the import to abort.
Does NOT promote records to reviewed/ — use tools/review_tooling_records.py for that.
...
ADAPTER_OUTPUT = (
    ...
    / "tooling_adapters"
    / "output"
    / "guhring_sample_records.json"
)
DEFAULT_OUTPUT = RECORDS_DIR / "guhring_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_guhring_sample.py' first.")
        return 1
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import Guhring adapter output into the Enterprise Tooling Search index."
    )
    parser.add_argument(
        "--output",
