"""Import Mitsubishi Materials adapter output into the Enterprise Tooling Search index.

Reads:
    tools/tooling_adapters/output/mitsubishi_materials_sample_records.json

Validates using the standard tooling import workflow, then writes to:
    tool_data/tooling_search/records/mitsubishi_materials_imported_tools.json

This script is the second stage of the adapter pipeline, after the adapter runner
has produced its output.  It does NOT skip the validation step: forbidden feed/speed
fields cause the import to abort.  It does NOT promote records to reviewed/ — use
tools/review_tooling_records.py for that step.

Usage::

    python tools/import_mitsubishi_adapter_output.py
    python tools/import_mitsubishi_adapter_output.py --dry-run
"""
...
ADAPTER_OUTPUT = (
    ...
    / "tooling_adapters"
    / "output"
    / "mitsubishi_materials_sample_records.json"
)
DEFAULT_OUTPUT = RECORDS_DIR / "mitsubishi_materials_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_mitsubishi_materials_sample.py' first.")
        return 1
    ...
    print("  2. Run: python tools/review_tooling_records.py <path> ...")
    print("     to promote records to records/reviewed/.")
    return 0


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import Mitsubishi adapter output into the Enterprise Tooling Search index."
    )
    parser.add_argument(
        "--output",
