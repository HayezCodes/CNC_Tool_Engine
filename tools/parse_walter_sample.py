"""Runner script: parse the Walter AG sample JSON fixture.

Parses tools/tooling_adapters/samples/sample_walter_structured.json,
validates the output against the tooling search schema, and writes normalized
JSON records to:

    tools/tooling_adapters/output/walter_sample_records.json
...
Usage::
    ...
    python tools/parse_walter_sample.py --output path/to/custom.json
"""
...
from tools.tooling_adapters.walter_adapter import parse_walter_file
...
SAMPLE_JSON = (
    ...
    / "tooling_adapters"
    / "samples"
    / "sample_walter_structured.json"
)
DEFAULT_OUTPUT = (
    Path(__file__).resolve().parent
    / "tooling_adapters"
    / "output"
    / "walter_sample_records.json"
)
...
def run(output_path: Path, dry_run: bool = False) -> int:
    ...
    result = parse_walter_file(SAMPLE_JSON)
    ...
    for rec in result["records"]:
        ...
        print(
            f"  [{rec['tool_category']:22s}] {rec['manufacturer_part_number']:36s} "
            f"  {status} / {cds}"
        )
    ...
    print()
    print("Next steps:")
    print("  1. Run: python tools/import_walter_adapter_output.py")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse the Walter AG sample JSON fixture and write normalized JSON output."
    )
    parser.add_argument(
        "--output",
