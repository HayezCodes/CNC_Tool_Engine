"""Import Tungaloy adapter output into the Enterprise Tooling Search index.
...
ADAPTER_OUTPUT = _REPO_ROOT / "tools" / "tooling_adapters" / "output" / "tungaloy_sample_records.json"
DEFAULT_OUTPUT = RECORDS_DIR / "tungaloy_imported_tools.json"
...
def run(output_path: Path, dry_run: bool = False) -> int:
    if not ADAPTER_OUTPUT.exists():
        ...
        print("Run 'python tools/parse_tungaloy_sample.py' first.")
        return 1
