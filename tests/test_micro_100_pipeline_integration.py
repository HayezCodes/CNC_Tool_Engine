"""Integration tests: Micro 100 adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "micro_100_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "micro_100_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "micro_100_reviewed_tooling_records.json"

BRAND = "Micro 100"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-M100-BB-001", "FIXTURE-M100-BB-002", "FIXTURE-M100-GI-003", "FIXTURE-M100-GI-004",
    "FIXTURE-M100-THI-005", "FIXTURE-M100-TM-006", "FIXTURE-M100-SCE-007", "FIXTURE-M100-CSK-008",
}
...
class TestSearchable:
    ...
    def test_search_by_micro_100(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("Micro 100")
        assert results and any(r["brand"] == BRAND for r in results)

    def test_filter_by_brand(self):
        from grade_engine.tooling_search import load_tooling_records, filter_tooling_records
        results = filter_tooling_records(load_tooling_records(), {"brand": "micro 100"})
        assert len(results) >= 8

    def test_suggest_boring_bar_candidates(self):
        from grade_engine.tooling_search import suggest_tool_candidates
        results = suggest_tool_candidates("boring", "P", tool_category="boring_bar", limit=20)
