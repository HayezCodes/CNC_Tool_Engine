...
from tools.stage_kyocera_families import OUTPUT_FILENAME, build_kyocera_family_records
...
def test_all_records_are_kyocera() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Kyocera"}
...
def test_required_kyocera_families_exist() -> None:
    records = _load_staged_records()
    ...
    assert {"turning_insert", "milling_insert", "grooving_insert", "threading_insert", "indexable_drill"}.issubset(categories)
    assert {"external_turning", "face_milling", "grooving", "threading", "drilling"}.issubset(operations)
