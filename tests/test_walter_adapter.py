"""Unit tests for the Walter AG structured JSON adapter.
...
from tools.tooling_adapters.walter_adapter import WalterAdapter, parse_walter_file
...
SAMPLE_JSON_PATH = (
    ...
    / "tools" / "tooling_adapters" / "samples" / "sample_walter_structured.json"
)
...
def _make_record(**overrides) -> dict:
    base = {
        "part_number": "FIXTURE-WAL-TEST-001",
        "tool_type": "TurningInsert",
        "series": "Tiger·tec Silver Fixture Series",
        "family_name": "Test turning insert family",
        "designation": "CNMG 120408-NF4",
        "grade": "FIXTURE-WAP35S",
        "chipbreaker": "FIXTURE-NF4",
...
class TestWalterAdapterInlineJson:
    ...
    def test_brand_always_set_from_header(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(_minimal_json([_make_record()]))
        assert records[0]["brand"] == "Walter AG"
    ...
    def test_chipbreaker_field_preserved(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(
            _minimal_json([_make_record(chipbreaker="FIXTURE-NF4")])
        )
        assert records[0]["chipbreaker"] == "FIXTURE-NF4"
    ...
    def test_source_page_reference_preserved(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(_minimal_json([_make_record()]))
        assert records[0]["source_page_reference"] == "p.C-1"
    ...
    def test_operation_fit_milling_maps_correctly(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(
            _minimal_json([_make_record(operations=["ShoulderMilling", "FaceMilling", "SlotMilling"])])
        )
        assert "shoulder_milling" in records[0]["operation_fit"]
        assert "face_milling" in records[0]["operation_fit"]
        assert "slot_milling" in records[0]["operation_fit"]
    ...
    def test_tool_category_shoulder_milling_insert_maps_to_milling_insert(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(
            _minimal_json([_make_record(tool_type="ShoulderMillingInsert")])
        )
        assert records[0]["tool_category"] == "milling_insert"

    def test_tool_category_face_milling_insert_maps_to_milling_insert(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse_json_string(
            _minimal_json([_make_record(tool_type="FaceMillingInsert")])
        )
        assert records[0]["tool_category"] == "milling_insert"
...
class TestSampleFileIntegration:
    ...
    def test_sample_all_brands_are_walter(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse(SAMPLE_JSON_PATH)
        for record in records:
            assert record["brand"] == "Walter AG"
    ...
    def test_sample_shoulder_milling_insert_maps_to_milling_insert(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse(SAMPLE_JSON_PATH)
        smi = next(r for r in records if r["manufacturer_part_number"] == "FIXTURE-WAL-SMI-002")
        assert smi["tool_category"] == "milling_insert"

    def test_sample_shoulder_milling_insert_has_shoulder_milling_operation(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse(SAMPLE_JSON_PATH)
        smi = next(r for r in records if r["manufacturer_part_number"] == "FIXTURE-WAL-SMI-002")
        assert "shoulder_milling" in smi["operation_fit"]

    def test_sample_solid_carbide_drill_maps_to_drill(self) -> None:
        adapter = WalterAdapter()
        records = adapter.parse(SAMPLE_JSON_PATH)
        scd = next(r for r in records if r["manufacturer_part_number"] == "FIXTURE-WAL-SCD-004")
        assert scd["tool_category"] == "drill"
    ...
    def test_parse_walter_file_convenience_function(self) -> None:
        result = parse_walter_file(SAMPLE_JSON_PATH)
