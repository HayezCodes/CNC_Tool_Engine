...
from tools.stage_helical_families import OUTPUT_FILENAME, build_helical_family_records
...
def test_all_records_are_helical_solutions() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Helical Solutions"}
...
def test_dynamic_adaptive_milling_family_exists() -> None:
    records = _load_staged_records()

    assert any("dynamic_milling" in record["operation_fit"] for record in records)
    assert any("adaptive" in record["strategy_fit"] for record in records)


def test_aluminum_family_exists() -> None:
    records = _load_staged_records()

    assert any("aluminum_milling" in record["operation_fit"] for record in records)
    assert any(record["material_fit"] == ["N"] for record in records)


def test_chamfer_family_exists() -> None:
    records = _load_staged_records()

    assert any(record["tool_category"] == "chamfer_mill" for record in records)
    assert any("chamfer" in record["operation_fit"] for record in records)
