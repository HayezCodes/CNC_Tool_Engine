...
from tools.stage_sumitomo_electric_families import OUTPUT_FILENAME, build_sumitomo_electric_family_records
...
def test_all_records_are_sumitomo_electric() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Sumitomo Electric"}
...
def test_required_sumitomo_electric_families_exist() -> None:
    records = _load_staged_records()
