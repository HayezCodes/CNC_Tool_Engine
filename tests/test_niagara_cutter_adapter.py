"""Unit tests for the Niagara Cutter structured JSON adapter."""
from __future__ import annotations
import json
from pathlib import Path
from tools.tooling_adapters.niagara_cutter_adapter import NiagaraCutterAdapter, parse_niagara_cutter_file
...
SAMPLE_JSON_PATH = (
    ...
    / "tools" / "tooling_adapters" / "samples" / "sample_niagara_cutter_structured.json"
)
...
def _make_record(**overrides):
    base = {
        "part_number": "FIXTURE-NIA-TEST-001", "tool_type": "SolidCarbideEndmill",
        "series": "N55000", "family_name": "Test endmill", "designation": "N55004",
...
class TestNiagaraCutterAdapterInline:
    def test_empty_returns_empty(self):
        assert NiagaraCutterAdapter().parse_json_string(_minimal_json([])) == []

    def test_single_record_parses(self):
        assert len(NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record()]))) == 1

    def test_brand_from_header(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record()]))
    ...
    def test_material_iso_only(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(material_groups=["P", "Z"])]))
        assert records[0]["material_fit"] == ["P"]

    def test_tool_category_endmill(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideEndmill")]))
        assert records[0]["tool_category"] == "endmill"

    def test_tool_category_drill(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideDrill")]))
        assert records[0]["tool_category"] == "drill"

    def test_tool_category_tap(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(tool_type="SpiralTap")]))
        assert records[0]["tool_category"] == "tap"

    def test_tool_category_thread_mill(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(tool_type="ThreadMill")]))
        assert records[0]["tool_category"] == "thread_mill"

    def test_tool_category_reamer(self):
        records = NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideReamer")]))
        assert records[0]["tool_category"] == "reamer"

    def test_forbidden_feed_key_rejects(self):
        a = NiagaraCutterAdapter(); bad = _make_record(); bad["feed_rate"] = 0.2
        assert a.parse_json_string(_minimal_json([bad])) == [] and a.rejected_count == 1

    def test_no_forbidden_keys_in_output(self):
        for r in NiagaraCutterAdapter().parse_json_string(_minimal_json([_make_record()])):
            for k in r:
                for t in FORBIDDEN: assert t not in k.lower()

    def test_validate_output_passes(self):
        a = NiagaraCutterAdapter()
...
class TestNiagaraCutterSampleFile:
    def test_sample_exists(self): assert SAMPLE_JSON_PATH.exists()

    def test_sample_parses_no_errors(self):
        a = NiagaraCutterAdapter(); a.parse(SAMPLE_JSON_PATH)
        assert a.parse_errors == []

    def test_sample_expected_count(self):
        assert len(NiagaraCutterAdapter().parse(SAMPLE_JSON_PATH)) == EXPECTED_RECORD_COUNT
    ...
    def test_sample_all_schema_fields(self):
        for r in NiagaraCutterAdapter().parse(SAMPLE_JSON_PATH):
            assert all(f in r for f in SCHEMA_FIELDS)

    def test_sample_all_brands_correct(self):
        for r in NiagaraCutterAdapter().parse(SAMPLE_JSON_PATH): assert r["brand"] == BRAND
    ...
    def test_sample_covers_expected_categories(self):
        expected = {"endmill", "drill", "reamer", "thread_mill", "tap"}
        found = {r["tool_category"] for r in NiagaraCutterAdapter().parse(SAMPLE_JSON_PATH)}
        assert expected.issubset(found), f"Missing: {expected - found}"

    def test_convenience_function(self):
        result = parse_niagara_cutter_file(SAMPLE_JSON_PATH)
