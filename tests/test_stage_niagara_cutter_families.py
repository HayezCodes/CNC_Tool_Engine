...
from tools.stage_niagara_cutter_families import OUTPUT_FILENAME, build_niagara_cutter_family_records
...
def test_all_records_are_niagara_cutter() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Niagara Cutter"}
...
def test_required_niagara_cutter_families_exist() -> None:
    records = _load_staged_records()
    ...
    assert {"general_milling", "roughing", "finishing", "contouring"}.issubset(operations)
    assert {"production_milling", "high_performance", "surface_finish_priority"}.issubset(strategies)
