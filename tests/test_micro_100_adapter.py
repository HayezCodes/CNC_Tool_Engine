"""Unit tests for the Micro 100 structured JSON adapter."""
from __future__ import annotations
import json
from pathlib import Path
from tools.tooling_adapters.micro_100_adapter import Micro100Adapter, parse_micro_100_file
...
SAMPLE_JSON_PATH = (
    ...
    / "tools" / "tooling_adapters" / "samples" / "sample_micro_100_structured.json"
)
...
def _make_record(**overrides):
    base = {
        "part_number": "FIXTURE-M100-TEST-001", "tool_type": "SolidCarbideBoringBar",
        "series": "QB", "family_name": "Test boring bar", "designation": "QB-010-10",
        "grade": "Solid Carbide", "chipbreaker": "", "coating": "Uncoated",
        "material_groups": ["P", "M"], "operations": ["Boring"],
        "geometry_tags": ["solid_carbide_shank"], "holder_compatibility": "VDI tool holders",
...
class TestMicro100AdapterInline:
    def test_empty_returns_empty(self):
        assert Micro100Adapter().parse_json_string(_minimal_json([])) == []

    def test_single_record_parses(self):
        assert len(Micro100Adapter().parse_json_string(_minimal_json([_make_record()]))) == 1

    def test_brand_from_header(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record()]))
    ...
    def test_material_iso_only(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(material_groups=["P", "Z"])]))
        assert records[0]["material_fit"] == ["P"]

    def test_tool_category_boring_bar(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideBoringBar")]))
        assert records[0]["tool_category"] == "boring_bar"

    def test_tool_category_boring_bar_from_boring_bar(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="BoringBar")]))
        assert records[0]["tool_category"] == "boring_bar"

    def test_tool_category_grooving_insert(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="ExternalGroovingInsert")]))
        assert records[0]["tool_category"] == "grooving_insert"

    def test_tool_category_threading_insert(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="ExternalThreadingInsert")]))
        assert records[0]["tool_category"] == "threading_insert"

    def test_tool_category_thread_mill(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="ThreadMill")]))
        assert records[0]["tool_category"] == "thread_mill"

    def test_tool_category_endmill(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideEndmill")]))
        assert records[0]["tool_category"] == "endmill"

    def test_tool_category_countersink(self):
        records = Micro100Adapter().parse_json_string(_minimal_json([_make_record(tool_type="Countersink")]))
        assert records[0]["tool_category"] == "countersink"

    def test_forbidden_feed_key_rejects(self):
        a = Micro100Adapter(); bad = _make_record(); bad["feed_rate"] = 0.2
        assert a.parse_json_string(_minimal_json([bad])) == [] and a.rejected_count == 1

    def test_no_forbidden_keys_in_output(self):
        for r in Micro100Adapter().parse_json_string(_minimal_json([_make_record()])):
            for k in r:
...
class TestMicro100SampleFile:
    ...
    def test_sample_all_schema_fields(self):
        for r in Micro100Adapter().parse(SAMPLE_JSON_PATH):
            assert all(f in r for f in SCHEMA_FIELDS)

    def test_sample_all_brands_correct(self):
        for r in Micro100Adapter().parse(SAMPLE_JSON_PATH): assert r["brand"] == BRAND
    ...
    def test_sample_covers_expected_categories(self):
        expected = {"boring_bar", "grooving_insert", "threading_insert", "thread_mill",
                    "endmill", "countersink"}
        found = {r["tool_category"] for r in Micro100Adapter().parse(SAMPLE_JSON_PATH)}
        assert expected.issubset(found), f"Missing: {expected - found}"

    def test_convenience_function(self):
        result = parse_micro_100_file(SAMPLE_JSON_PATH)
