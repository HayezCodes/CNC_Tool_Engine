...
from tools.stage_tungaloy_families import OUTPUT_FILENAME, build_tungaloy_family_records
...
def test_all_records_are_tungaloy() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Tungaloy"}
...
def test_required_tungaloy_families_exist() -> None:
    records = _load_staged_records()
    ...
    assert {
        ...
        "grooving_toolholder",
        "threading_insert",
        "threading_toolholder",
        "indexable_cutter",
        "multifunction_tooling",
    }.issubset(categories)
    assert {"grooving", "threading", "high_feed_milling", "parting"}.issubset(operations)
