"""Integration tests: YG-1 adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "yg1_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "yg1_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "yg_1_reviewed_tooling_records.json"

BRAND = "YG-1"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-YG1-SCD-001", "FIXTURE-YG1-SCE-002", "FIXTURE-YG1-SCE-003", "FIXTURE-YG1-TAP-004",
    "FIXTURE-YG1-TM-005", "FIXTURE-YG1-RMR-006", "FIXTURE-YG1-CSK-007",
    "FIXTURE-YG1-TI-008", "FIXTURE-YG1-GI-009",
}
...
class TestSearchable:
    ...
    def test_search_by_yg1(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("YG-1")
    ...
    def test_filter_by_brand(self):
        ...
        results = filter_tooling_records(load_tooling_records(), {"brand": "yg-1"})
