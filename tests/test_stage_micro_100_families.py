...
from tools.stage_micro_100_families import OUTPUT_FILENAME, build_micro_100_family_records
...
def test_all_records_are_micro_100() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Micro 100"}
...
def test_required_micro_100_families_exist() -> None:
    records = _load_staged_records()
    ...
    assert {
        "boring_bar",
        "miniature_turning_tool",
        "grooving_tool",
        "threading_tool",
        "specialty_turning",
    }.issubset(categories)
    assert {"boring", "internal_turning", "internal_grooving", "internal_threading", "small_feature_turning"}.issubset(
        operations
    )
