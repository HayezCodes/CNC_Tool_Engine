"""Integration tests: Harvey Tool adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "harvey_tool_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "harvey_tool_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "harvey_tool_reviewed_tooling_records.json"

BRAND = "Harvey Tool"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-HVT-SCE-001", "FIXTURE-HVT-SCE-002", "FIXTURE-HVT-ITM-003", "FIXTURE-HVT-ETM-004",
    "FIXTURE-HVT-RMR-005", "FIXTURE-HVT-CSK-006", "FIXTURE-HVT-SCD-007", "FIXTURE-HVT-STP-008",
}
...
class TestSearchable:
    ...
    def test_search_by_harvey(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("Harvey")
    ...
    def test_filter_by_brand(self):
        ...
        results = filter_tooling_records(load_tooling_records(), {"brand": "harvey tool"})
        assert len(results) >= 8

    def test_suggest_endmill_candidates(self):
        from grade_engine.tooling_search import suggest_tool_candidates
        results = suggest_tool_candidates("profiling", "P", tool_category="endmill", limit=20)
