"""Integration tests: Garr Tool adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "garr_tool_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "garr_tool_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "garr_tool_reviewed_tooling_records.json"

BRAND = "Garr Tool"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-GRT-SCE-001", "FIXTURE-GRT-SCE-002", "FIXTURE-GRT-SCE-003", "FIXTURE-GRT-SCE-004",
    "FIXTURE-GRT-SCD-005", "FIXTURE-GRT-RMR-006", "FIXTURE-GRT-TM-007", "FIXTURE-GRT-CSK-008",
}
...
class TestSearchable:
    ...
    def test_search_by_garr(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("Garr Tool")
    ...
    def test_filter_by_brand(self):
        ...
        results = filter_tooling_records(load_tooling_records(), {"brand": "garr"})
