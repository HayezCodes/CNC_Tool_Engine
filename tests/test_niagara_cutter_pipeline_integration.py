"""Integration tests: Niagara Cutter adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "niagara_cutter_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "niagara_cutter_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "niagara_cutter_reviewed_tooling_records.json"

BRAND = "Niagara Cutter"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-NIA-SCE-001", "FIXTURE-NIA-SCE-002", "FIXTURE-NIA-SCE-003", "FIXTURE-NIA-SCE-004",
    "FIXTURE-NIA-SCD-005", "FIXTURE-NIA-RMR-006", "FIXTURE-NIA-TM-007", "FIXTURE-NIA-TAP-008",
}
...
class TestSearchable:
    ...
    def test_search_by_niagara(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("Niagara Cutter")
    ...
    def test_filter_by_brand(self):
        ...
        results = filter_tooling_records(load_tooling_records(), {"brand": "niagara"})
