"""Integration tests: Seco Tools adapter → import → audit → review → search."""
from __future__ import annotations
import json
from pathlib import Path

_REPO = Path(__file__).resolve().parent.parent
_ADAPTER_OUTPUT = _REPO / "tools" / "tooling_adapters" / "output" / "seco_sample_records.json"
_IMPORTED = _REPO / "tool_data" / "tooling_search" / "records" / "seco_imported_tools.json"
_REVIEWED = _REPO / "tool_data" / "tooling_search" / "records" / "reviewed" / "seco_tools_reviewed_tooling_records.json"

BRAND = "Seco Tools"
FORBIDDEN = {"feed", "speed", "sfm", "rpm", "ipr", "ipm", "vc", "fz"}
EXPECTED_MPNS = {
    "FIXTURE-SECO-TI-001", "FIXTURE-SECO-TI-002", "FIXTURE-SECO-MI-003", "FIXTURE-SECO-HFI-004",
    "FIXTURE-SECO-SCD-005", "FIXTURE-SECO-SCE-006", "FIXTURE-SECO-GI-007",
    "FIXTURE-SECO-THI-008", "FIXTURE-SECO-BB-009",
}
...
class TestSearchable:
    ...
    def test_search_by_seco(self):
        from grade_engine.tooling_search import search_tooling_records
        results = search_tooling_records("Seco")
    ...
    def test_filter_by_brand(self):
        ...
        results = filter_tooling_records(load_tooling_records(), {"brand": "seco"})
        assert len(results) >= 9

    def test_filter_turning_insert(self):
        from grade_engine.tooling_search import load_tooling_records, filter_tooling_records
        results = filter_tooling_records(load_tooling_records(), {"brand": "seco", "tool_category": "turning_insert"})
