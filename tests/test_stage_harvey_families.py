...
from tools.stage_harvey_families import OUTPUT_FILENAME, build_harvey_family_records
...
def test_all_records_are_harvey_tool() -> None:
    assert {record["brand"] for record in _load_staged_records()} == {"Harvey Tool"}
...
def test_required_harvey_specialty_families_exist() -> None:
    records = _load_staged_records()
    ...
    assert {"chamfer", "keyseat", "threading", "miniature_milling", "undercutting", "specialty_milling"}.issubset(operations)
