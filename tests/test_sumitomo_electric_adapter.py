"""Unit tests for the Sumitomo Electric structured JSON adapter."""
from __future__ import annotations
import json
from pathlib import Path
from tools.tooling_adapters.sumitomo_electric_adapter import SumitomoElectricAdapter, parse_sumitomo_electric_file
...
SAMPLE_JSON_PATH = (
    ...
    / "tools" / "tooling_adapters" / "samples" / "sample_sumitomo_electric_structured.json"
)
...
def _make_record(**overrides):
    base = {
        "part_number": "FIXTURE-SMT-TEST-001", "tool_type": "TurningInsert",
        "series": "AC/WBN", "family_name": "Test turning insert", "designation": "CNMG 431 EG",
        "grade": "AC8035P", "chipbreaker": "EG", "coating": "PVD TiAlSiN",
...
class TestSumitomoElectricAdapterInline:
    def test_empty_returns_empty(self):
        assert SumitomoElectricAdapter().parse_json_string(_minimal_json([])) == []

    def test_single_record_parses(self):
        assert len(SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record()]))) == 1

    def test_brand_from_header(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record()]))
    ...
    def test_material_iso_only(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record(material_groups=["P", "Z"])]))
        assert records[0]["material_fit"] == ["P"]

    def test_tool_category_turning_insert(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record(tool_type="TurningInsert")]))
        assert records[0]["tool_category"] == "turning_insert"

    def test_tool_category_high_feed_insert(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record(tool_type="HighFeedMillingInsert")]))
        assert records[0]["tool_category"] == "high_feed_insert"

    def test_tool_category_indexable_drill(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record(tool_type="IndexableDrill")]))
        assert records[0]["tool_category"] == "indexable_drill"

    def test_tool_category_endmill(self):
        records = SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record(tool_type="SolidCarbideEndmill")]))
        assert records[0]["tool_category"] == "endmill"

    def test_forbidden_feed_key_rejects(self):
        a = SumitomoElectricAdapter(); bad = _make_record(); bad["feed_rate"] = 0.2
        assert a.parse_json_string(_minimal_json([bad])) == [] and a.rejected_count == 1

    def test_no_forbidden_keys_in_output(self):
        for r in SumitomoElectricAdapter().parse_json_string(_minimal_json([_make_record()])):
            for k in r:
                for t in FORBIDDEN: assert t not in k.lower()

    def test_validate_output_passes(self):
        a = SumitomoElectricAdapter()
...
class TestSumitomoElectricSampleFile:
    def test_sample_exists(self): assert SAMPLE_JSON_PATH.exists()

    def test_sample_parses_no_errors(self):
        a = SumitomoElectricAdapter(); a.parse(SAMPLE_JSON_PATH)
        assert a.parse_errors == []

    def test_sample_expected_count(self):
        assert len(SumitomoElectricAdapter().parse(SAMPLE_JSON_PATH)) == EXPECTED_RECORD_COUNT
    ...
    def test_sample_all_schema_fields(self):
        for r in SumitomoElectricAdapter().parse(SAMPLE_JSON_PATH):
            assert all(f in r for f in SCHEMA_FIELDS)

    def test_sample_all_brands_correct(self):
        for r in SumitomoElectricAdapter().parse(SAMPLE_JSON_PATH): assert r["brand"] == BRAND
    ...
    def test_sample_covers_expected_categories(self):
        ...
        found = {r["tool_category"] for r in SumitomoElectricAdapter().parse(SAMPLE_JSON_PATH)}
        assert expected.issubset(found), f"Missing: {expected - found}"

    def test_convenience_function(self):
        result = parse_sumitomo_electric_file(SAMPLE_JSON_PATH)
